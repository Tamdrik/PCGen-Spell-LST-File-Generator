...
class Spell:
    def __init__(self, name: str, classes_by_level: list, school: str, casting_time: str, spell_range: str,
                 ...
                 divine_focus: bool = False, other_fields: list = ()):
...
class SpellGenerator:
    def __init__(self, spells=[], mods=[]):
        """
        Initialize the SpellGenerator class mainly by building all the GUI elements.
        ...
        self.spell_list_label = Label(self.spell_list_frame, text="Spells")
        ...
        spell_edit_frame = Frame(self.win, width=800)
        spell_edit_frame.pack(side=RIGHT, fill=BOTH, expand=True)
        ...
        for i in range(0, rows):
            spell_edit_subframes.append(Frame(spell_edit_frame))
        for subframe in spell_edit_subframes:
            subframe.pack(side=TOP, fill=BOTH, expand=True, pady=4)

        row = 0
        self.spell_labels['name'] = Label(spell_edit_subframes[row], text="Spell Name", font='bold')
        self.spell_labels['name'].pack(side=LEFT)
        ...
        self.spell_labels['school'] = Label(spell_edit_subframes[row], text="School")
        self.spell_labels['school'].pack(side=LEFT)
        self.schools = ("Abjuration", "Conjuration", "Divination", "Enchantment", "Evocation", "Illusion", "Necromancy",
                        "Transmutation", "Universal")
        self.selected_school = StringVar(self.win)
        ...
        self.type_cb['arcane'] = Checkbutton(self.type_frame, text="Arcane", variable=self.type_values['arcane'],
                                             onvalue=True, offvalue=False)
        self.type_cb['arcane'].pack(side=LEFT)
        self.type_cb['divine'] = Checkbutton(self.type_frame, text="Divine", variable=self.type_values['divine'],
                                             onvalue=True, offvalue=False)
        self.type_cb['divine'].pack(side=LEFT)
        self.type_cb['psychic'] = Checkbutton(self.type_frame, text="Psychic", variable=self.type_values['psychic'],
                                              onvalue=True, offvalue=False)
        self.type_cb['psychic'].pack(side=LEFT)
        ...
        self.component_cb['verbal'] = Checkbutton(self.components_frame, text="V",
                                                  variable=self.component_values['verbal'],
                                                  onvalue=True, offvalue=False)
        self.component_cb['verbal'].pack(side=LEFT)
        self.component_cb['somatic'] = Checkbutton(self.components_frame, text="S",
                                                   variable=self.component_values['somatic'],
                                                   onvalue=True, offvalue=False)
        self.component_cb['somatic'].pack(side=LEFT)
        self.component_cb['material'] = Checkbutton(self.components_frame, text="M",
                                                    variable=self.component_values['material'],
                                                    onvalue=True, offvalue=False)
        self.component_cb['material'].pack(side=LEFT)
        self.component_cb['focus'] = Checkbutton(self.components_frame, text="F",
                                                 variable=self.component_values['focus'],
                                                 onvalue=True, offvalue=False)
        self.component_cb['focus'].pack(side=LEFT)
        self.component_cb['divine_focus'] = Checkbutton(self.components_frame, text="DF",
                                                        variable=self.component_values['divine_focus'],
                                                        onvalue=True, offvalue=False)
        self.component_cb['divine_focus'].pack(side=LEFT)
        ...
        self.spell_labels['subschool'] = Label(spell_edit_subframes[row], text="Subschool")
        self.spell_labels['subschool'].pack(side=LEFT, padx=(15, 1))
        #self.spell_fields['subschool'] = Entry(spell_edit_subframes[row])
        #self.spell_fields['subschool'].pack(side=LEFT)
        ...
        self.classes = ("Alchemist", "Antipaladin", "Bard", "Bloodrager", "Cleric", "Druid", "Hunter", "Inquisitor",
                        "Magus", "Medium", "Mesmerist", "Occultist", "Paladin", "Psychic", "Ranger", "Shaman",
                        "Spiritualist", "Summoner", "Witch", "Wizard")
        self.caster_type = {}
        self.caster_type['arcane'] = ("Alchemist", "Bard", "Bloodrager", "Magus", "Summoner", "Witch", "Wizard")
        self.caster_type['divine'] = ("Antipaladin", "Cleric", "Druid", "Hunter", "Inquisitor", "Paladin", "Ranger", "Shaman")
        self.caster_type['psychic'] = ("Medium", "Mesmerist", "Occultist", "Psychic", "Spiritualist")
        self.selected_class = StringVar(self.win)
        self.selected_class.set("Alchemist")
        ...
        self.descriptors = ["Acid", "Air", "Chaotic", "Cold", "Curse", "Darkness", "Death", "Disease", "Draconic",
                            ...
                            "Language-Dependent", "Lawful", "Light", "Meditative", "Mind-Affecting", "Pain", "Poison",
                            "Shadow", "Sonic", "Water"]
        ...
        row = row + 1
        self.spell_labels['casting_time'] = Label(spell_edit_subframes[row], text="Casting Time")
        self.spell_labels['casting_time'].pack(side=LEFT)
        ...
        self.spell_labels['range'] = Label(spell_edit_subframes[row], text="Range")
        self.spell_labels['range'].pack(side=LEFT, padx=(15, 1))
        ...
        self.spell_labels['duration'] = Label(spell_edit_subframes[row], text="Duration")
        self.spell_labels['duration'].pack(side=LEFT, padx=(15, 1))
        ...
        row = row + 1
        self.spell_labels['target'] = Label(spell_edit_subframes[row], text="Target")
        self.spell_labels['target'].pack(side=LEFT)
        ...
        self.spell_labels['save'] = Label(spell_edit_subframes[row], text="Save")
        self.spell_labels['save'].pack(side=LEFT, padx=(15, 1))
        ...
        sr_frame = LabelFrame(spell_edit_subframes[row], text="SR?")
        sr_frame.pack(side=LEFT, padx=(15, 10))
        self.sr_values = ("Yes", "Yes (Harmless)", "No", "None")
        self.selected_sr = StringVar(self.win)
        self.selected_sr.set("None")
        self.sr_dropdown = OptionMenu(sr_frame, self.selected_sr, *self.sr_values)
    ...
    def add_class(self) -> None:
        ...
        if not class_already_in_list:
            ...
            if class_name in self.caster_type['arcane']:
                self.type_cb['arcane'].select()
            elif class_name in self.caster_type['divine']:
                self.type_cb['divine'].select()
            elif class_name in self.caster_type['psychic']:
                self.type_cb['psychic'].select()

            # If Wizard is added, also add Sorcerer, since that appears to be PCGen .lst convention
            if class_string.count("Wizard") > 0:
    ...
    def populate_fields(self, spell: Spell) -> None:
        ...
        for descriptor in spell.descriptors:
            self.descriptors_lb.insert(0, descriptor)
        for spell_type in spell.type.keys():
            if spell.type[spell_type]:
                self.type_cb[spell_type].select()
            else:
                self.type_cb[spell_type].deselect()
        for component in spell.comps.keys():
        ...
        if filename is not None and len(filename) > 0:
            ...
            if os.path.isfile(filename):
                ...
                if header.upper().count("HOMEBREW") == 0 and header.upper().count("MPC") == 0:
                    ...
                    if not answer:
                        return
            self.generate_spell_lst(filename=filename, spells=self.spell_list, mods=self.mod_list)
    ...
    @staticmethod
    def generate_pcc_file(pcc_file: str, spell_lst_file: str) -> bool:
        """
        ...
        try:
            ...
            with open(pcc_file, "w") as f:
                ...
                choices = ("Pathfinder", "D&D 3.5e")
                system = StringVar(mode_dialog)
                system.set("Pathfinder")
    ...
    def load_spells(self) -> None:
        ...
        if filename is not None and len(str(filename)) > 0:
            ...
            self.mod_list.clear()
            (self.header, self.spell_list, self.mod_list) = self.load_spell_lst(filename=filename)
    ...
    def generate_spell_lst(filename: str, spells: list, mods: list = (),
                           header: str = "SOURCELONG:Homebrew\tSOURCESHORT:Homebrew\tSOURCEWEB:None\t#\tSOURCEDATE:" +
                                         str(datetime.datetime.now()).split(" ")[0]) -> None:
        """
