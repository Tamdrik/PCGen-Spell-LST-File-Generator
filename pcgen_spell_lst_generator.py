...
v1.1.3: Checks for empty "required" fields before outputting associated token, since there are some oddball spell
        ...
        types and components that caused crashes in newer versions of Python. 
v1.1.4: When user saves spells to a .lst, will now attempt to validate that the chosen directory falls within the
        PCGen data folder, where it belongs.  If not, it will alert the user that PCGen may not be able to find/load
        it.  Adds "generated by" comment to output files.
"""

PCGEN_TAB_SIZE = 6  # Used to format field spacing when writing to a .lst file
VERSION = "1.1.4"
BUILD_DATE = "15 July 2022"
...
class SpellGenerator:
    ...
    def generate_spell_lst(filename: str, spells: list, mods: list = (),
        ...
        with open(filename, "w") as f:
            f.write("# Generated by PCGen Spell LST File Generator " +
                    "(https://github.com/Tamdrik/PCGen-Spell-LST-File-Generator)\n")
            f.write(header + "\n")
            f.write("\n")
            for spell in spells:
                spell.mode = mode
...
class SpellEditor(Frame):
    ...
    def add_descriptor(self, event=None) -> None:
    ...
    def update_material_desc_field(self) -> None:
        if self.mode == "D&D 5e":
